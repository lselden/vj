...
Interface.Panel = function() {
  ...
  Interface.extend(this, {
        ...
        var event = {
          ...
          type: e.type,
				...
				if(e.offsetX == null) {
					...
					event.x = e.pageX - offset.left;
					event.y = e.pageY - offset.top;
				}
    ...
    draw : function(immediate) {
      if (!this.active) return; // skip
			
			if(!immediate && this.shouldDraw.length) {
				return requestAnimationFrame(this.draw.bind(this, true));
			}
    ...
    refresh: function(immediate) {
      if(this.active) {
				if(immediate) {	
					...
					window.requestAnimationFrame(this.refresh.bind(this, true));
				}
      }
    },
  ...
  this.init();
	...
	var boundDraw = this.draw.bind(this, true); // for requestAnimationFrame
	var boundRequest = window.requestAnimationFrame.bind(window, boundDraw);
	
  this.timer = setInterval(boundRequest, Math.round(1000 / this.fps));
...
Interface.XY = function() {
  ...
  Interface.extend(this, {
    ...
    startAnimation : function() { 
			var boundRefresh = this.refresh.bind(this, true);
			var boundRequest = window.requestAnimationFrame.bind(window, boundRefresh);
      if(this.timer === null) { 
        this.timer = setInterval( boundRequest, (1 / this.fps) * 1000); 
      } 
    },
      ...
      if(this.hasFocus || !this.requiresFocus) {
        ...
        if( buttonHit !== this.mouseOver ) {
          ...
          if(this.values[ buttonHit ] !== this.lastValues[ buttonHit ] || this.mode === 'contact') {
            ...
            if(this.mode === 'contact') {
              var self = this;
              setTimeout( function() { self._values[ buttonHit ] = 0;
